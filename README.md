...
Proximity offers two complementary musical modes. Both derive a loose, linked, or tight coupling level from the smoothed surface gap between every phrase pair, with stable changes taking effect at the next eligible unscheduled bar. **Lock Rhythm** snaps nearby phrases toward half-beat or beat grids without changing their notes, durations, or dynamics. **Accent Together** leaves timing and density untouched and emphasizes notes that already begin together. The musician chooses the behavior, and mode changes take effect on the next bar without interrupting playback.
